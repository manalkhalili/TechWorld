...
<head>
    <meta charset="UTF-8">
    <title>Keyboard and mouses</title>
...
?>
<!--navbar end-->
<h1 class = "lg-title">Keyboard and mouse set</h1>
<div class = "products" style="background: white; ">
    <div class = "containerr">
        <div class = "product-items">
            <?php
            $sql="SELECT * FROM product WHERE Type='keyboard and mouse'";
            $result = $db->query($sql);
            while($row= $result->fetch_assoc()) {?>
                <form action="KeyboardandMouse.php? code=<?=$row['Code']?>" method="post">
                    <?php
                    $Name=$row['Name'];
                    $Price=$row['Price'];
                    $Image=$row['Image'];
